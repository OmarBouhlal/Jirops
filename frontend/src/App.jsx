...
import { loadSession, onSessionChange, saveSession } from './lib/storage';
...
import { Key, Mail, Lock, Award, Compass, Sparkles } from 'lucide-react';
...
function App() {
  ...
  useEffect(() => {
    if (!session?.accessToken || activeTab !== 'settings' || !selectedProjectId) {
      return;
    }
    ...
    async function loadUsers() {
      setBusy((current) => ({ ...current, users: true }));
      try {
        const nextUsers = await api.listUsers(session.accessToken);
        if (!cancelled) {
        ...
        if (!cancelled) {
          setBusy((current) => ({ ...current, users: false }));
        }
      }
    }
  ...
  }, [activeTab, selectedProjectId, session?.accessToken, users.length]);
  ...
  async function handleTaskSubmit(event) {
    event.preventDefault();
    const projectId = taskForm.projectId || selectedProjectId;
    if (!projectId) return;
    ...
    try {
      ...
      setTaskForm({ ...emptyTask, projectId, sprintId: taskForm.sprintId });
  ...
  function openTaskDrawer(projectId = selectedProjectId) {
    setTaskForm((prev) => ({
      ...prev,
      projectId: projectId || '',
      assignee: '',
            ...
            <main className="min-h-[500px]">
              ...
              {activeTab === 'board' && (
                <BoardView
                  ...
                  onCreateTaskClick={() => openTaskDrawer()}
                />
              )}
