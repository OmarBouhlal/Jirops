...
function TaskCard({ task, onStatusChange, onDelete, userDirectory }) {
  const assigneeLabel = formatUserLabel(task.assignee, userDirectory);
    ...
    <article className="group relative rounded-2xl border border-white/5 bg-slate-900/60 p-4 shadow-lg hover:border-indigo-500/20 hover:bg-slate-900/80 transition-all duration-200">
      ...
      <div className="mt-4 flex items-center justify-between border-t border-white/5 pt-3 text-[10px] text-slate-400">
        <div className="flex items-center gap-3">
          {/* Assignee Avatar */}
          <div className="flex items-center gap-1.5" title={`Assignee: ${assigneeLabel}`}>
...
export function BoardView({ tasks, onStatusChange, onDelete, onCreateTaskClick, activeProject, userDirectory }) {
  const [searchQuery, setSearchQuery] = useState('');
  const [priorityFilter, setPriorityFilter] = useState('');
  const normalizedQuery = searchQuery.trim().toLowerCase();

  const filteredTasks = useMemo(() => {
    if (!normalizedQuery && !priorityFilter) {
      return tasks;
    }
  ...
  }, [tasks, normalizedQuery, priorityFilter]);
