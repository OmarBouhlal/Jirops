...
export function SprintsView({ 
  ...
  const { activeSprints, plannedSprints, closedSprints, backlogTasks, tasksBySprintId } = useMemo(() => {
    const grouped = {
      activeSprints: [],
      plannedSprints: [],
      closedSprints: [],
      backlogTasks: [],
      tasksBySprintId: new Map(),
    };

    for (const task of tasks) {
      if (task.sprintId) {
        const current = grouped.tasksBySprintId.get(task.sprintId);
        if (current) current.push(task);
        else grouped.tasksBySprintId.set(task.sprintId, [task]);
      } else {
        grouped.backlogTasks.push(task);
      }
    }

    for (const sprint of sprints) {
      if (sprint.status === 'ACTIVE') grouped.activeSprints.push(sprint);
      else if (sprint.status === 'PLANNED') grouped.plannedSprints.push(sprint);
      else grouped.closedSprints.push(sprint);
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex gap-2">
              {sprint.status === 'PLANNED' && (
                <button
                  type="button"
                  onClick={() => handleSprintAction(api.startSprint, sprint)}
            ...
            {sprintTasks.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse text-xs">
                  ...
                  <tbody>
                    {sprintTasks.map(t => {
                      const assigneeLabel = formatUserLabel(t.assignee, userDirectory);
                      const avatar = getAvatarDetails(assigneeLabel);
                      return (
          ...
          <div className="space-y-3 overflow-y-auto pr-1 flex-1">
            {backlogTasks.map((task) => {
              const priority = (value) => {
                if (value === 'LOW') return 'text-slate-400 bg-slate-500/10';
                if (value === 'MEDIUM') return 'text-cyan-400 bg-cyan-500/10';
                if (value === 'HIGH') return 'text-amber-400 bg-amber-500/10';
